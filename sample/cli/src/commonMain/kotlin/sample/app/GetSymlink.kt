...
// java -jar cli-jvm.jar GetSymlink --region `region` --bucket `bucket` --key `key`
class GetSymlink : SampleSubcommand(
    "GetSymlink",
    ...
    override suspend fun executeCommand() {
        OSSClient.create(ClientConfiguration.loadDefault().apply {
            ...
            val res = client.getSymlink(GetSymlinkRequest {
                this.bucket = argBucket
                key = argKey
            })
            println("symlinkTarget: ${res.symlinkTarget}")
        }
    }
}