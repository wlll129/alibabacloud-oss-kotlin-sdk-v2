...
// java -jar cli-jvm.jar IsBucketExist --region `region` --bucket `bucket`
class IsBucketExist :
    SampleSubcommand("IsBucketExist", "Queries all buckets that are owned by a requester.") {
    ...
    override suspend fun executeCommand() {
        OSSClient.create(ClientConfiguration.loadDefault().apply {
            ...
            val isBucketExist = client.doesBucketExist(bucket = argBucket)
            println(
                "$argBucket is ${
                    if (isBucketExist) {
                        ""
