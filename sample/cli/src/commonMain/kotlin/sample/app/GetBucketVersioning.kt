...
// java -jar cli-jvm.jar GetBucketVersioning --region `region` --bucket `bucket`
class GetBucketVersioning :
    SampleSubcommand("GetBucketVersioning", "Queries the versioning state of a bucket.") {
    ...
    override suspend fun executeCommand() {
        OSSClient.create(ClientConfiguration.loadDefault().apply {
            ...
            val res = client.getBucketVersioning(GetBucketVersioningRequest {
                this.bucket = argBucket
            })
            println("versioning status: ${res.versioningConfiguration?.status}")
        }
    }
}