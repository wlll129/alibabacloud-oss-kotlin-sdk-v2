...
public class VectorsSignerV4(
    ...
    override suspend fun sign(signingContext: SigningContext) {
        if (accountId == null) {
            throw SignatureException("VectorsSignerV4.accountId is null")
        }
    ...
    private fun buildCanonicalUri(signingCtx: SigningContext): String {
        val sb = StringBuilder("/acs:ossvector:${signingCtx.region}:")

        sb.append(signingCtx.bucket?.let {
            if (it.isNotEmpty()) {
                "$accountId:${signingCtx.bucket}/"
            } else {
                ":/"
            }
        } ?: ":/")
